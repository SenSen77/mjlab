...
class MotionLoader:
  def __init__(
    ...
    dof_mapping: tuple[int, ...] | None = None,
  ):
  ...
  def _load_motion(self):
    ...
    if self.dof_mapping is not None:
      cols = torch.tensor(self.dof_mapping, device=self.device, dtype=torch.long)
      if cols.numel() == 0:
        raise ValueError("dof_mapping is empty.")
      if int(cols.max().item()) >= self.motion_dof_poss_input.shape[1]:
        raise ValueError(
          "dof_mapping contains out-of-range index. "
          f"{self.motion_dof_poss_input.shape[1]=}, {int(cols.max().item())=}."
        )
      self.motion_dof_poss_input = self.motion_dof_poss_input[:, cols]
  ...
  while not file_saved:
    ...
    if not file_saved:
      ...
      if reset_flag and not file_saved:
        ...
        if not upload_wandb:
          print("[INFO] upload_wandb=False, skipping WandB upload.")
          return
        ...
        run = wandb.init(project=wandb_project, name=COLLECTION, entity=wandb_entity)
        print(f"[INFO]: Logging motion to wandb: {COLLECTION}")
        REGISTRY = "motions"
        logged_artifact = run.log_artifact(
          artifact_or_path=out_npz, name=COLLECTION, type=REGISTRY
        )
        run.link_artifact(
...
def main(
  ...
  render: bool = False,
  line_range: tuple[int, int] | None = None,
  task_id: str = "Mjlab-Tracking-Flat-Unitree-G1-23DoF",
  dof_mapping: tuple[int, ...] | None = (
    0,
    1,
    2,
    3,
  ...
  Args:
    ...
    task_id: mjlab task id used to build the robot scene.
    dof_mapping: Optional mapping that selects/reorders DOF columns from the CSV
      (after the first 7 root columns). Defaults to the rl_sar g1_23 mapping from
      23DOF joint order to 29DOF CSV order.
    upload_wandb: Whether to upload the generated motion.npz to WandB registry.
    wandb_project: WandB project to log the conversion run under.
    wandb_entity: WandB entity/team to log under (overrides WANDB_ENTITY env var).
  """
